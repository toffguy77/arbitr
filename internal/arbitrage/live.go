...
func usdPriceOfAsset(ctx context.Context, by common.ExchangeAdapter, asset string) (float64, error) {
	...
	if a == "USDT" || a == "USD" { return 1.0, nil }
...
func (e *Engine) executeTriangle(ctx context.Context, by common.ExchangeAdapter, tri config.Triangle, dir string, tAB, tBC, tCA common.Ticker, slipBps float64, netBps float64) error {
	...
	if qtyUSD <= 0 { qtyUSD = e.cfg.Trading.NotionalUSD }
	if qtyUSD <= 0 { qtyUSD = 50 }

	// Adaptive sizing by edge: more edge -> more size (cap at MaxNotionalUSD)
	edgeBps := netBps
	mult := 1.0
if edgeBps > 0 { mult = 1.0 + math.Min(edgeBps/10.0, 1.0) } // up to 2x
...
if netBps > e.cfg.Trading.MinNetBps {
	...
	getFilters := func(sym string) (qStep, pStep, minQty, minNotional float64) {
		...
		if fp, ok := by.(common.SymbolFiltersProvider); ok {
			...
			if qs, ps, mq, mn, ok2 := fp.GetSymbolFilters(ctxS, sym); ok2 { qStep, pStep, minQty, minNotional = qs, ps, mq, mn }
			cancelS()
		} else if st, ok := by.(common.SymbolStepper); ok {
			ctxS, cancelS := context.WithTimeout(ctx, 2*time.Second)
			if qs, ps, ok2 := st.GetSymbolSteps(ctxS, sym); ok2 { qStep, pStep = qs, ps }
			cancelS()
		}
		return
	}
	...
	type leg struct{ sym string; side common.OrderSide; px float64 }
	legs := make([]leg, 0, 3)
	switch dir {
	case "F":
	...
	if len(legs) != 3 { return fmt.Errorf("invalid legs") }
	first := legs[0]
	if first.px <= 0 { return fmt.Errorf("invalid first leg price") }
	qty := qtyUSD / first.px
	...
	if ob, ok := by.(common.OrderbookProvider); ok {
		...
		if ok1 && ok2 && ok3 {
			build := func(bids [][2]float64, asks [][2]float64) orderbook.L2 { var l2 orderbook.L2; for _, b := range bids { l2.Bids = append(l2.Bids, orderbook.Level{Price:b[0], Qty:b[1]}) }; for _, a := range asks { l2.Asks = append(l2.Asks, orderbook.Level{Price:a[0], Qty:a[1]}) }; return l2 }
			...
			if cap3 < cap { cap = cap3 }
			if cap > 0 && cap < qty { qty = cap }
		}
	}
...
if qty <= 0 || first.px <= 0 { return fmt.Errorf("qty/price invalid after rounding") }
if minQty > 0 && qty < minQty { return fmt.Errorf("first leg qty below minQty") }
if minNotional > 0 && qty*first.px < minNotional { return fmt.Errorf("first leg notional below minNotional") }

	// Submit first leg
	ctx1, cancel1 := context.WithTimeout(ctx, 5*time.Second)
	id1, err := by.PlaceOrder(ctx1, common.Order{Symbol: first.sym, Side: first.side, Qty: qty, Price: first.px, TimeInForce: "FOK"})
	metrics.ExecutionLatencyMs.Observe(float64(time.Since(startDecision).Milliseconds()))
	cancel1()
	if err != nil || id1 == "" { return fmt.Errorf("first leg rejected: %w", err) }
	...
	if qs, ok := by.(common.OrderStatusQuerier); ok {
		...
		cS()
		if err == nil {
if f < qty { metrics.PartialFillsTotal.Inc() }
			filled1 = f
...
if minQty2 > 0 && qty2 < minQty2 { return fmt.Errorf("second leg qty below minQty") }
if minNot2 > 0 && qty2*second.px < minNot2 { return fmt.Errorf("second leg notional below minNotional") }
	if e.riskEng != nil {
		...
		} else { if adjQty2 > 0 { qty2 = adjQty2 } }
	}
	...
	if err != nil || id2 == "" {
		...
		opSide := common.Sell; if first.side == common.Sell { opSide = common.Buy }
...
if minQty3 > 0 && qty3 < minQty3 { return fmt.Errorf("third leg qty below minQty") }
if minNot3 > 0 && qty3*third.px < minNot3 { return fmt.Errorf("third leg notional below minNotional") }
	if qs, ok := by.(common.OrderStatusQuerier); ok {
		ctxS2, cS2 := context.WithTimeout(ctx, 2*time.Second)
		st2, f2, err2 := qs.GetOrderStatus(ctxS2, id2)
		cS2()
		if err2 == nil {
if f2 < qty2 { metrics.PartialFillsTotal.Inc() }
			qty3 = roundDown(f2, qStep3)
			if st2 != "Filled" && st2 != "PartiallyFilled" {
				if canc, ok2 := by.(common.OrderCanceler); ok2 { _ = canc.CancelOrder(ctx, id2) }
				// unwind first
				op1 := common.Sell; if first.side == common.Sell { op1 = common.Buy }
	...
	if e.riskEng != nil {
		if ok, reason, adjQty3, _ := e.riskEng.Allow(risk.OrderRequest{Symbol: third.sym, Side: third.side, Qty: qty3, Price: third.px, Exchange: by.Name(), ValueUSD: qty3 * third.px}); !ok {
			...
			e.unwindPair(ctx, by, first.sym, func() common.OrderSide { if first.side == common.Sell { return common.Buy }; return common.Sell }(), qty, first.px)
			return fmt.Errorf("risk blocked third leg: %s (unwind attempted)", reason)
		} else { if adjQty3 > 0 { qty3 = adjQty3 } }
	}
	...
	if e.riskEng != nil {
		...
		if fee1 > 0 { feeUSD += convFee(fee1, first.sym) }
		if fee2 > 0 { feeUSD += convFee(fee2, second.sym) }
		if fee3 > 0 { feeUSD += convFee(fee3, third.sym) }
		...
		// base USD prices
		b1USD := avg1
		if b1USD <= 0 { b1USD = first.px }
		b1USD = b1USD * usd1
		b2USD := avg2
		if b2USD <= 0 { b2USD = second.px }
		b2USD = b2USD * usd2
		b3USD := avg3
		if b3USD <= 0 { b3USD = third.px }
		b3USD = b3USD * usd3
		// leg1
...
func (e *Engine) unwindPair(ctx context.Context, by common.ExchangeAdapter, symbol string, side common.OrderSide, qty float64, refPrice float64) {
	...
	skew := maxBps / 10000.0
	price := refPrice
	if side == common.Buy { price = refPrice * (1 + skew) } else { price = refPrice * (1 - skew) }
