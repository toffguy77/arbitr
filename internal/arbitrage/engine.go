...
func New(cfg config.Config, adapters map[string]common.ExchangeAdapter, logger log.Logger) *Engine {
	...
	limits := risk.Limits{
		...
		SoftStopLossUSD: func() float64 { if cfg.Trading.DailyPnLStopUSD>0 { return cfg.Trading.DailyPnLStopUSD*0.5 }; return 0 }(),
...
func (e *Engine) Run(ctx context.Context) error {
	...
	if len(triangles) == 0 {
		...
		// After triangles prepared, prefetch symbol steps if adapter supports it
			...
			if feeder, ok := by.(common.LiveMarketFeeder); ok {
				syms := func() []string { out := make([]string, 0, len(allSyms)); for s := range allSyms { out = append(out, s) }; return out }()
	...
	go func(){
		...
		by := e.adapters["bybit"]
		for {
select { case <-ctx.Done(): return; case <-upd.C:
				if by == nil { continue }
				balancer, ok := by.(common.Balancer); if !ok { continue }
				ctxTO, cancel := context.WithTimeout(ctx, 5*time.Second)
				balances, err := balancer.GetBalances(ctxTO); cancel()
				if err != nil { continue }
				var totalUSD float64
				for _, b := range balances {
					if strings.EqualFold(b.Asset, "USDT") || strings.EqualFold(b.Asset, "USD") { totalUSD += b.Free; continue }
					// try to price asset in USDT
					tick, err2 := by.GetTicker(ctx, strings.ToUpper(b.Asset)+"USDT"); if err2 == nil { totalUSD += b.Free * tick.Bid }
				}
				if e.riskEng != nil { e.riskEng.UpdatePortfolioValue(totalUSD) }
			}
		}
	}()
	...
	for {
		...
		case <-t.C:
			...
			if elapsed > 0 {
				...
				if e.tokens > float64(e.cfg.Trading.MaxOrdersPerMin) { e.tokens = float64(e.cfg.Trading.MaxOrdersPerMin) }
			...
			for _, tri := range triangles {
				...
				go func() {
					...
					cancel()
					if eAB != nil || eBC != nil || eCA != nil { return }

					// Estimate dynamic slippage for tentative qty
					qtyBase := e.cfg.Trading.MaxNotionalUSD
					if qtyBase <= 0 { qtyBase = e.cfg.Trading.NotionalUSD }
					if qtyBase <= 0 { qtyBase = 50 }
					qtyAB := qtyBase / maxf(tAB.Bid, tAB.Ask)
					slipBps := e.estimateTriangleSlippageBps(ctx, by, tri, qtyAB, tAB, tBC, tCA, "F")
					...
					net := netF; gross := grossF; dir := "F"
					if netR > netF { net = netR; gross = grossR; dir = "R" }
					...
					e.mu.Lock()
					if until, ok := e.triBan[key]; ok && time.Now().Before(until) { e.mu.Unlock(); return }
					// dynamic threshold per triangle
					minBps := e.dynamicMinNetBps(key)
					if net >= minBps { e.consec[key]++ } else { e.consec[key] = 0 }
					if until, ok := e.cooldown[key]; ok && time.Now().Before(until) { e.mu.Unlock(); return }
...
if net >= minBps {
					...
					if e.riskEng != nil && e.riskEng.IsHardStopped() { e.mu.Lock(); e.stopped = true; e.mu.Unlock() }
					e.mu.Lock(); stopped := e.stopped; e.mu.Unlock()
					if e.cfg.Trading.Live && !stopped && e.allowedForLive(tri) {
						// check token bucket: need up to 1 unit per attempt (conservative)
						e.mu.Lock(); allow := e.tokens >= 1; if allow { e.tokens -= 1 } ; e.mu.Unlock()
						if !allow { return }
						// try acquire concurrency slot without blocking
						select { case e.execSem <- struct{}{}: default: return }
						defer func(){ <-e.execSem }()
						metrics.TrianglesAttemptedTotal.Inc()
if err := e.executeTriangle(ctx, by, tri, dir, tAB, tBC, tCA, slipBps, net); err != nil {
							...
							if e.cfg.Trading.TriangleCooldownSeconds > 0 { e.mu.Lock(); e.cooldown[key] = time.Now().Add(time.Duration(e.cfg.Trading.TriangleCooldownSeconds) * time.Second); e.mu.Unlock() }
						}
							}
						select {
...
func (e *Engine) updateTriStats(key string, net float64, success bool, partial bool) {
	...
	defer e.mu.Unlock()
	st := e.triStats[key]
	if st == nil { st = &triStat{}; e.triStats[key] = st }
	st.count++
	if success { st.success++ }
	if partial { st.partials++ }
	// EMA with alpha=0.2
	alpha := 0.2
	if st.count == 1 { st.emaNet = net } else { st.emaNet = alpha*net + (1-alpha)*st.emaNet }
...
func (e *Engine) dynamicMinNetBps(key string) float64 {
    ...
    ratio := 0.0
    if st.count > 0 { ratio = float64(st.success)/float64(st.count) }
    bump := 0.0
    // if success ratio < 50%, add up to +5 bps
    if ratio < 0.5 { bump += (0.5 - ratio) * 10.0 }
    if bump > 5.0 { bump = 5.0 }
    // if EMA below base, add half the gap, capped
    if st.emaNet < base { bump += (base - st.emaNet) * 0.5 }
    if bump > 10.0 { bump = 10.0 }
out := base + bump
    return out
}
...
func (e *Engine) estimateTriangleSlippageBps(ctx context.Context, by common.ExchangeAdapter, tri config.Triangle, qtyAB float64, tAB, tBC, tCA common.Ticker, dir string) float64 {
	...
	if !ok { return e.cfg.Trading.SlippageBps }
	// Helper to build L2
	build := func(bids [][2]float64, asks [][2]float64) orderbook.L2 {
		l2 := orderbook.L2{}
		for _, b := range bids { l2.Bids = append(l2.Bids, orderbook.Level{Price:b[0], Qty:b[1]}) }
		for _, a := range asks { l2.Asks = append(l2.Asks, orderbook.Level{Price:a[0], Qty:a[1]}) }
		return l2
	}
	...
	if !ok1 || !ok2 || !ok3 { return e.cfg.Trading.SlippageBps }
