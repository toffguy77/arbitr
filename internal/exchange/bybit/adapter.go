...
func (a *Adapter) GetSymbolFilters(ctx context.Context, symbol string) (float64, float64, float64, float64, bool) {
	...
	defer func(){ _ = resp.Body.Close() }()
var r struct{ Result struct{ List []struct{ LotSizeFilter struct{ StepSize string `json:"stepSize"`; BasePrecision string `json:"basePrecision"`; MinOrderQty string `json:"minOrderQty"` } `json:"lotSizeFilter"`; PriceFilter struct{ TickSize string `json:"tickSize"`; MinOrderAmt string `json:"minOrderAmt"` } `json:"priceFilter"` } `json:"list"` } `json:"result"` }
	if err := json.NewDecoder(resp.Body).Decode(&r); err != nil { return 0,0,0,0,false }
	if len(r.Result.List)==0 { return 0,0,0,0,false }
	parse := func(s string) float64 { var f float64; _,_ = fmt.Sscan(s, &f); return f }
	it := r.Result.List[0]
	qs := parse(it.LotSizeFilter.StepSize)
	if qs==0 { qs = parse(it.LotSizeFilter.BasePrecision) }
	ps := parse(it.PriceFilter.TickSize)
	minQty := parse(it.LotSizeFilter.MinOrderQty)
	minNotional := parse(it.PriceFilter.MinOrderAmt)
	a.mu.Lock(); a.filters[symbol] = struct{ qtyStep, priceStep, minQty, minNotional float64 }{qtyStep: qs, priceStep: ps, minQty: minQty, minNotional: minNotional}; a.mu.Unlock()
...
func (a *Adapter) PlaceOrder(ctx context.Context, ord common.Order) (string, error) {
	...
	if fp, ok := any(a).(common.SymbolFiltersProvider); ok {
		...
		if qs, ps, mq, mn, ok2 := fp.GetSymbolFilters(ctxS, ord.Symbol); ok2 {
			...
			if ord.Qty <= 0 || ord.Price <= 0 { cancelS(); return "", fmt.Errorf("qty/price invalid after rounding") }
			if mq > 0 && ord.Qty < mq { cancelS(); return "", fmt.Errorf("qty below minQty") }
			if mn > 0 && ord.Qty*ord.Price < mn { cancelS(); return "", fmt.Errorf("notional below minNotional") }
		}
		cancelS()
	...
	body := map[string]string{
		...
		"timeInForce": func() string { if ord.TimeInForce != "" { return ord.TimeInForce }; return "FOK" }(),
	}
	if ord.ID != "" { body["orderLinkId"] = ord.ID }
...
func (a *Adapter) GetOrderStatus(ctx context.Context, orderID string) (string, float64, error) {
	...
	start := time.Now()
	resp, err := a.http.Do(req)
	if err != nil { a.logger.Debug().Err(err).Str("method", http.MethodGet).Str("endpoint", endpoint).Msg("http request failed"); return "", 0, err }
	defer func(){ _ = resp.Body.Close() }()
	var r struct{
		RetCode int `json:"retCode"`
		RetMsg string `json:"retMsg"`
		Result struct{ List []struct{ OrderStatus string `json:"orderStatus"`; CumExecQty string `json:"cumExecQty"` } `json:"list"` } `json:"result"`
	}
...
func (a *Adapter) GetOrderFillInfo(ctx context.Context, orderID string) (avgPrice float64, cumFee float64, filledQty float64, err error) {
	...
	var r struct{
		RetCode int `json:"retCode"`
		RetMsg string `json:"retMsg"`
		Result struct{ List []struct{ AvgPrice string `json:"avgPrice"`; CumExecFee string `json:"cumExecFee"`; CumExecQty string `json:"cumExecQty"` } `json:"list"` } `json:"result"`
	}
	if err := json.NewDecoder(resp.Body).Decode(&r); err != nil { return 0,0,0, err }
	if resp.StatusCode < 200 || resp.StatusCode >= 300 || r.RetCode != 0 || len(r.Result.List) == 0 { return 0,0,0, fmt.Errorf("fillinfo http=%d ret=%d", resp.StatusCode, r.RetCode) }
...
func (a *Adapter) CancelOrder(ctx context.Context, orderID string) error {
	if a.cfg.Exchanges.Bybit.APIKey == "" || a.cfg.Exchanges.Bybit.Secret == "" { return fmt.Errorf("bybit api credentials not set") }
	...
	if err != nil { return err }
	defer func(){ _ = resp.Body.Close() }()
	var r struct{ RetCode int `json:"retCode"`; RetMsg string `json:"retMsg"` }
	_ = json.NewDecoder(resp.Body).Decode(&r)
	if resp.StatusCode < 200 || resp.StatusCode >= 300 || r.RetCode != 0 { return fmt.Errorf("cancel failed http=%d ret=%d msg=%s", resp.StatusCode, r.RetCode, r.RetMsg) }
	return nil
}
...
dial := func(ctx context.Context) (*websocket.Conn, *http.Response, error) { return websocket.DefaultDialer.DialContext(ctx, wsURL, nil) }
	...
	c.SetPongHandler(func(string) error { a.mu.Lock(); a.lastWS = time.Now(); a.mu.Unlock(); return nil })
		...
		if err := c.WriteJSON(sub); err != nil { c.Close(); return err }
		// ping ticker
		ping := time.NewTicker(15*time.Second)
		go func(){ for { select { case <-ctx.Done(): return; case <-ping.C: _ = c.WriteControl(websocket.PingMessage, []byte("ping"), time.Now().Add(2*time.Second)) } } }()
		// staleness updater
		go func() { t := time.NewTicker(1 * time.Second); defer t.Stop(); for { select { case <-ctx.Done(): return; case <-t.C: a.mu.RLock(); last := a.lastWS; a.mu.RUnlock(); if !last.IsZero() { metrics.BookStalenessMs.WithLabelValues("bybit").Set(float64(time.Since(last).Milliseconds())) } } } }()
		// reader
		go func(){ defer c.Close(); for { select { case <-ctx.Done(): return; default:
var msg struct{ Topic string `json:"topic"`; Type string `json:"type"`; Data json.RawMessage `json:"data"` }
			if err := c.ReadJSON(&msg); err != nil { return }
			if strings.HasPrefix(msg.Topic, "tickers.") {
				var t struct{ Bid1 string `json:"bid1Price"`; Ask1 string `json:"ask1Price"` }
				_ = json.Unmarshal(msg.Data, &t)
				var bid, ask float64; _, _ = fmt.Sscan(t.Bid1, &bid); _, _ = fmt.Sscan(t.Ask1, &ask)
				sym := strings.TrimPrefix(msg.Topic, "tickers."); a.mu.Lock(); a.tickers[sym] = common.Ticker{Bid:bid, Ask:ask}; a.lastWS = time.Now(); a.mu.Unlock()
			} else if strings.HasPrefix(msg.Topic, "orderbook.1.") {
				var ob struct{ Bids [][]string `json:"b"`; Asks [][]string `json:"a"` }
				_ = json.Unmarshal(msg.Data, &ob)
				parse := func(s string) float64 { var f float64; _, _ = fmt.Sscan(s, &f); return f }
bids := make([][2]float64, 0, len(ob.Bids)); for _, lv := range ob.Bids { if len(lv) >= 2 { bids = append(bids, [2]float64{parse(lv[0]), parse(lv[1])}) } }
				asks := make([][2]float64, 0, len(ob.Asks)); for _, lv := range ob.Asks { if len(lv) >= 2 { asks = append(asks, [2]float64{parse(lv[0]), parse(lv[1])}) } }
...
if len(bids) > 0 && len(asks) > 0 && bids[0][0] >= asks[0][0] {
					...
					cancelSnap()
if ok { bids, asks = b2, a2 }
				}
				a.mu.Lock(); a.books[sym] = struct{ bids, asks [][2]float64 }{bids:bids, asks:asks}; a.lastWS = time.Now(); a.mu.Unlock()
			}
		} } }()
		return nil
	}
...
if err := run(ctx, topics); err != nil {
	...
	go func(){
		...
		for {
			...
			a.mu.RLock(); stale := time.Since(a.lastWS) > 5*time.Second; a.mu.RUnlock()
			if stale {
				metrics.WSReconnectsTotal.WithLabelValues("bybit", "stale").Inc()
				_ = run(ctx, topics)
			}
			time.Sleep(backoff)
			if backoff < 8*time.Second { backoff *= 2 } else { backoff = 8*time.Second }
		}
	}()
	return nil
}
